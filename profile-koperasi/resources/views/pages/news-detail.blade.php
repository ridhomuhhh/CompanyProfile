...
@section('content')
  ...
  <div class="container-fluid p-0 mb-5">
    ...
    @php
      $bgImage = Str::startsWith($berita->gambar, 'http') ? $berita->gambar : asset('storage/' . $berita->gambar);
    @endphp

    <div
      style="background: url('{{ $bgImage }}') no-repeat center center; background-size: cover; height: 450px; position: relative;">
      <!-- Overlay Gelap -->
      <div
        style="position: absolute; top:0; left:0; width:100%; height:100%; background: linear-gradient(to bottom, rgba(0,0,0,0.3), rgba(0,0,0,0.8));">
      </div>

      <div class="container h-100 d-flex align-items-end pb-5 position-relative">
        <div class="text-white col-lg-8">
          {{-- <span class="badge bg-primary mb-3 px-3 py-2">kategori</span> --}}
          <h1 class="display-4 fw-bold">{{ $berita->judul_berita }}</h1>
          <div class="d-flex align-items-center mt-3 text-light">
            <span class="me-4"><i class="bi bi-calendar3 me-2"></i>
    ...
    <div class="row g-5">
      ...
      <div class="col-lg-8">
        ...
        <div class="mt-5 pt-4 border-top">
          <a href="{{ route('berita') }}" class="btn btn-outline-secondary px-4 rounded-pill">
            ...
            <div class="card-body p-4">
              ...
              <ul class="list-unstyled mb-0">
                ...
                @forelse($recentPosts as $recent)
                  <li class="mb-4">
                    <a href="{{ route('berita.detail', $recent->slug) }}"
                      class="d-flex text-decoration-none text-dark group-link">
                      {{-- Gambar Thumbnail --}}
                      @php
                        $thumbImage = Str::startsWith($recent->gambar, 'http')
                            ? $recent->gambar
                            : asset('storage/' . $recent->gambar);
                      @endphp
                      <img src="{{ $thumbImage }}" class="rounded-3 me-3 object-fit-cover" alt="Thumb"
                        style="width: 80px; height: 80px;">

                      <div>
                        <h6 class="fw-bold mb-1" style="font-size: 0.95rem; line-height: 1.4;">
                          {{ Str::limit($recent->judul_berita, 45) }}</h6>
